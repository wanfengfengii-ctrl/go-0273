...
func TestModel_RetryWaitsForLogicalSchedule(t *testing.T) {
	...
	cases := []testCase{
		...
		{name: "immediate retry is rejected", action: "retry", callID: "CALL1", op: "op-early-1", wantHTTP: http.StatusBadRequest, wantCode: domain.ErrorCode("RETRY_NOT_DUE"), wantStatus: pathogen.CallFailedRetry, wantAttempts: 1, wantFault: 1, wantNext: 1, wantError: pathogen.ErrDeviceRefused},
		{name: "repeated immediate retry is stable", action: "retry", callID: "CALL1", op: "op-early-2", wantHTTP: http.StatusBadRequest, wantCode: domain.ErrorCode("RETRY_NOT_DUE"), wantStatus: pathogen.CallFailedRetry, wantAttempts: 1, wantFault: 1, wantNext: 1, wantError: pathogen.ErrDeviceRefused},
		...
		{name: "retry before doubled backoff is rejected", action: "retry", callID: "CALL1", op: "op-early-3", wantHTTP: http.StatusBadRequest, wantCode: domain.ErrorCode("RETRY_NOT_DUE"), wantStatus: pathogen.CallFailedRetry, wantAttempts: 2, wantFault: 2, wantNext: 3, wantError: pathogen.ErrDeviceDisconnected},
